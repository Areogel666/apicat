...
export interface Project {
  ...
  p95_threshold_ms: number | null  // 1.0.5：压测 P95 参考线（项目级默认）
  p99_threshold_ms: number | null  // 1.0.5：压测 P99 参考线（项目级默认）
  created_at: string
  updated_at: string
}
...
export interface StressConfig {
  ...
  expect_status: string
  /** P95 参考线阈值 ms，null = 用默认 500（1.0.5） */
  p95_threshold_ms?: number | null
  /** P99 参考线阈值 ms，null = 用默认 1000（1.0.5） */
...
export interface ApiRequest {
  ...
  p95_threshold_ms: number | null  // 1.0.5：压测 P95 参考线（接口级覆盖）
  p99_threshold_ms: number | null  // 1.0.5：压测 P99 参考线（接口级覆盖）
  sort_order: number
  created_at: string
  updated_at: string
}
